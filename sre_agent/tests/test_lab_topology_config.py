...
FAULT_INJECTOR_CONFIG = REPO_ROOT / "fault_injector" / "fault-injector-test.yaml"
LAB_CONFIG = REPO_ROOT / "config.lab.yaml"
...
def test_lab_cli_config_tracks_fault_injector_inventory() -> None:
    inventory = _load_yaml(FAULT_INJECTOR_CONFIG)
    ...
    assert worker_names == {"worker-01", "worker-02", "worker-03"}
    ...
    assert "connected_to" not in ports["200GE1/0/4"]
