...
- Immediately verify injector/process evidence with `network.find_process` using pattern `tc|netem|fault_injector|fi_`.
- If qdisc output shows injected delay/loss/corruption rules, treat tc/netem residue as the primary root-cause candidate.
- If tc/netem evidence is already present (qdisc or process), do not conclude with evidence-insufficient wording.
...
- Only use `kill_process` when `network.find_process` (or equivalent direct evidence) confirms a still-running fault-injector process.
