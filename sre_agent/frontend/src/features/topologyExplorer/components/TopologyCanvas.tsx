...
const TopologyCanvas = forwardRef<TopologyCanvasHandle, TopologyCanvasProps>(function TopologyCanvas(
  ...
  useEffect(() => {
    onReadyStateChange?.(Boolean(instance));
  ...
  useImperativeHandle(
    ...
    () => ({
      ...
      exportView: () => {
        ...
        const liveNodes = instance.getNodes();
        ...
        const exportedNodes = flowNodes.map((node) => {

          const liveNode = liveNodeMap.get(node.id);

          const position = positionsRef.current[node.id] ?? liveNode?.position ?? node.position;

          const width = Number(liveNode?.width ?? node.width ?? NODE_WIDTH);

          const height = Number(liveNode?.height ?? node.height ?? NODE_HEIGHT);
          ...
          return {
            ...
            size: {
              ...
              width: Number.isFinite(width) ? width : NODE_WIDTH,

              height: Number.isFinite(height) ? height : NODE_HEIGHT,
            ...
            status: node.data.node.status,

            layer: node.data.node.layer,
        ...
        } satisfies TopologyCanvasExport;
