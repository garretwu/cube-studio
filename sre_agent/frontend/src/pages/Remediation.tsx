...
const STATUS_LABELS: Record<string, string> = {
  ...
  escalated: "已升级处理",
  escalation_required: "触发升级处理",
  execution_failed: "执行失败",
  execution_mocked: "演练执行",
...
function mapOneShotStatus(value?: string | null): string {
  const normalized = String(value ?? "").trim().toLowerCase();
  if (normalized === "resolved" || normalized === "execution_succeeded" || normalized === "success") {
    return "resolved";
  }
  if (normalized === "failed" || normalized === "execution_failed" || normalized === "rollback_failed") {
    return "failed";
  }
  if (normalized === "timeout" || normalized === "execution_timeout") {
    return "timeout";
  }
  if (normalized === "escalated" || normalized === "escalation_required") {
    return "escalated";
  }
  if (normalized === "rejected" || normalized === "approval_rejected") {
    return "rejected";
  }
  if (
    normalized === "remediating" ||
    normalized === "execution_started" ||
    normalized === "execution_mocked" ||
    normalized === "observation_started" ||
    normalized === "observation_result" ||
...
function getOneShotBatchStatus(overview?: RemediationOverview): { batch: string; progress: number; status: string }[] {
  if (!overview) return [];
  const totalSteps = Number(overview.progress.total_steps ?? 0);
  const completedSteps = Number(overview.progress.completed_steps ?? 0);
  const status = mapOneShotStatus(overview.progress.status);
  let progress = totalSteps > 0 ? Math.round((completedSteps / totalSteps) * 100) : 0;
  if (totalSteps <= 0 && status !== "pending" && status !== "validating") {
    progress = 100;
  }
  return [
    {
...
function getCanaryProgress(overview?: RemediationOverview): number | null {
  ...
  const batches = getOneShotBatchStatus(overview);
  if (batches.length > 0) {
    return Math.max(0, Math.min(100, Math.round(Number(batches[0]?.progress ?? 0))));
  }
  return 0;
}
                      ...
                      return (
                        <Fragment key={record.summary.session_id}>
                          ...
                          {isExpanded ? (() => {
                            ...
                            const expandedTimelineEvents = expandedEvents.filter((event) => isRemediationTimelineEvent(event));
                            ...
                            const displayBatches = getOneShotBatchStatus(expandedOverview);
                            const planSteps = expandedOverview?.plan.steps ?? [];
                            return (
                                  ...
                                  <div className="remediation-record-table__expand-panel">
                                    ...
                                    <div className="remediation-record-table__subsection">
                                      ...
                                      <div className="page-stack remediation-record-table__embedded-list">
                                        ...
                                        <CanaryProgress batches={displayBatches} />
                                      ...
                                      <div className="mini-card-list remediation-step-list remediation-record-table__embedded-list">
                                        {planSteps.length === 0 ? (
                                          <div className="mini-card remediation-empty-state remediation-empty-state--compact remediation-record-table__embedded-card">
                                            <p className="mini-card__title">暂无可执行步骤</p>
                                            <p className="mini-card__copy">当前会话还没有可执行修复计划，或计划未同步到修复页面。</p>
                                          </div>
                                        ) : (
                                          planSteps.map((step) => (
                                    ...
                                    <div className="remediation-record-table__subsection">
                                      ...
                                      <div className="mini-card-list remediation-timeline-list remediation-record-table__embedded-list">
                                        {expandedTimelineEvents.length === 0 ? (
                                          ...
                                          expandedTimelineEvents.map((event, index) => {
