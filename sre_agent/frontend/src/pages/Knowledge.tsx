...
import type { KnowledgeBaseScope, KnowledgeBaseSummary } from "../api/types";
...
import {
  ...
  formatKnowledgeScopeShortLabel,
  formatKnowledgeStatusLabel,
  formatStorageSize,
  getKnowledgeBasesByScope,
...
const SCOPE_OPTIONS: Array<{ value: KnowledgeBaseScope; label: string }> = [
  { value: "shared", label: "公共知识库" },
...
function KnowledgePage() {
  ...
  const [scope, setScope] = useState<KnowledgeBaseScope>("shared");
  ...
  const scopedKnowledgeBases = useMemo(() => getKnowledgeBasesByScope(knowledgeBases, scope), [knowledgeBases, scope]);
  const filteredKnowledgeBases = useMemo(
    () => filterKnowledgeBases(scopedKnowledgeBases, appliedFilters),
    [appliedFilters, scopedKnowledgeBases],
  );
  ...
  return (
    <div className="page-grid knowledge-page">
      <div className="page-intro">
        <SectionHeader
          title="知识库"
          description="统一查看知识库资产、文档索引状态与只读预览信息，便于按范围、状态和关键词快速定位目标资料。"
          actions={
      ...
      <SurfaceCard bodyClassName="knowledge-page__hero-body" variant="hero">
        <div className="knowledge-scope-tabs" role="tablist" aria-label="知识库范围切换">
          {SCOPE_OPTIONS.map((option) => (
            <button
              ...
              className={`knowledge-scope-tabs__tab${scope === option.value ? " knowledge-scope-tabs__tab--active" : ""}`}
        ...
        <div className="card-grid--metrics knowledge-page__metrics">
          <MetricTile hint="当前筛选范围内的知识库数量" label={`${formatKnowledgeScopeShortLabel(scope)}知识库`} value={metrics.baseCount} />
      ...
      <SurfaceCard className="knowledge-manage-workbench" description="按启用状态、索引状态和关键词过滤当前范围内的知识库。" title="筛选条件">
        ...
        {!errorMessage && isLoading ? (
          <div className="knowledge-page__state">
            ...
            <p className="skills-empty__description">正在同步当前范围下的知识库资产和索引状态。</p>
          </div>
        ) : null}
        ...
        {!errorMessage && !isLoading && filteredKnowledgeBases.length === 0 ? (
          <div className="knowledge-page__state">
            ...
            <p className="skills-empty__description">可以调整筛选条件后重新查询，或切换到另一个知识库范围查看。</p>
          </div>
        ) : null}
                ...
                {filteredKnowledgeBases.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="knowledge-table__row">
                      ...
                      <td>{formatKnowledgeScopeLabel(item.scope)}</td>
