...
> 完整 Channel 层设计见 [channel.md](channel.md)。

Fault Injector 使用以下 Channel：

| Channel | 用途 | 详见 |
|---------|------|------|
| **SSHChannel** | 节点命令执行 (stress-ng, tc netem) | channel.md §3.3 |
| **RedfishChannel** | BMC 硬件控制 (风扇, 重启) | channel.md §3.4 |
| **SwitchChannel** | 交换机端口/配置操作 | channel.md §3.5 |
| **K8sChannel** | Pod/Deployment 故障注入 | channel.md §3.6 |
| **CubeStudioChannel** | 推理服务状态查询与更新 | channel.md §3.1 |
| **PrometheusChannel** | 基线采集与偏差检测 | channel.md §3.2 |
